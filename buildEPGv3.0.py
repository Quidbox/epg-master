...
## by warlockPT
## altered by Quidbox for TBee
## v1.0 - 2015/04/10
## v3.0 - 2017/02/14 +compress file +upload to blob storage
##########################################
...
import os
try:
    os.remove(epgFilename + ".gz")
except OSError:
    pass

if log>0: print "COMPRESSING FILE"
import gzip
in_data = open(epgFilename, "rb").read()
gzf = gzip.open(epgFilename + ".gz", "wb")
gzf.write(in_data)
gzf.close()

try:
    os.remove(epgFilename)
except OSError:
    pass

if log>0: print "SENDING FILE"
from azure.storage.blob import BlockBlobService
block_blob_service = BlockBlobService(account_name='*****', account_key='****')

from azure.storage.blob import PublicAccess
block_blob_service.create_container('******', public_access=PublicAccess.Container)
